...
namespace Kethane.Particles
{
    public class ParticleSystemCfg
    {
        ...
        public ParticleSystemCfg(string name)
        {
            ...
            if (psystem == null) psystem = gameObject.AddComponent<ParticleSystem>();
        ...
        public ParticleSystem psystem { get; }
        public ParticleSystemRenderer psrenderer { get; }
        ...
        public static TEnum ParseEnum<TEnum>(ConfigNode node, string name, TEnum defaultValue)
        {
            ...
            {
                var enumStr = node.GetValue(name);
        ...
        public static ParticleSystem.MinMaxCurve ParseMinMaxCurve(ConfigNode node, string name,
            ParticleSystem.MinMaxCurve defaultValue)
        {
            if (node.HasValue(name))
            {
                float val;
        ...
        public static ParticleSystem.MinMaxGradient ParseMinMaxGradient(ConfigNode node, string name,
            ParticleSystem.MinMaxGradient defaultValue)
        {
            if (node.HasNode(name))
            {
        ...
        private void ParseMain(ConfigNode node)
        {
            var main = psystem.main;

            main.playOnAwake = Utils.ParseBool(node, "playOnAwake", main.playOnAwake);

            main.cullingMode = ParseEnum(node, "cullingMode", main.cullingMode);
            ...
            main.emitterVelocityMode = ParseEnum(node, "emitterVelocityMode", main.emitterVelocityMode);
            ...
            main.gravityModifierMultiplier =
                Utils.ParseFloat(node, "gravityModifierMultiplier", main.gravityModifierMultiplier);
            ...
            main.ringBufferMode = ParseEnum(node, "ringBufferMode", main.ringBufferMode);
            main.scalingMode = ParseEnum(node, "scalingMode", main.scalingMode);
            main.simulationSpace = ParseEnum(node, "simulationSpace", main.simulationSpace);
            ...
            main.startRotationXMultiplier =
                Utils.ParseFloat(node, "startRotationXMultiplier", main.startRotationXMultiplier);
            main.startRotationY = ParseMinMaxCurve(node, "startRotationY", main.startRotationY);
            main.startRotationYMultiplier =
                Utils.ParseFloat(node, "startRotationYMultiplier", main.startRotationYMultiplier);
            main.startRotationZ = ParseMinMaxCurve(node, "startRotationZ", main.startRotationZ);
            main.startRotationZMultiplier =
                Utils.ParseFloat(node, "startRotationZMultiplier", main.startRotationZMultiplier);
            ...
            main.stopAction = ParseEnum(node, "stopAction", main.stopAction);
            main.useUnscaledTime = Utils.ParseBool(node, "useUnscaledTime", main.useUnscaledTime);
        }

        private void ParseCollision(ConfigNode node)
        {
            ...
            mod.mode = ParseEnum(node, "mode", mod.mode);
            mod.multiplyColliderForceByCollisionAngle = Utils.ParseBool(node, "multiplyColliderForceByCollisionAngle",
                mod.multiplyColliderForceByCollisionAngle);
            mod.multiplyColliderForceByParticleSize = Utils.ParseBool(node, "multiplyColliderForceByParticleSize",
                mod.multiplyColliderForceByParticleSize);
            mod.multiplyColliderForceByParticleSpeed = Utils.ParseBool(node, "multiplyColliderForceByParticleSpeed",
                mod.multiplyColliderForceByParticleSpeed);
            mod.quality = ParseEnum(node, "quality", mod.quality);
            ...
            mod.type = ParseEnum(node, "type", mod.type);
            ...
            if (node.HasValue("planes"))
            {
                ...
                foreach (var p in planes)
                {
                    ...
                    if (go != null) mod.SetPlane(index++, go.transform);
                }
            }
        }

        private void ParseColorBySpeed(ConfigNode node)
        {
        ...
        private void ParseColorOverLifetime(ConfigNode node)
        {
        ...
        private void ParseEmission(ConfigNode node)
        {
            var mod = psystem.emission;

            mod.enabled = Utils.ParseBool(node, "enabled", true);
            mod.rateOverDistance = ParseMinMaxCurve(node, "rateOverDistance", mod.rateOverDistance);
            mod.rateOverDistanceMultiplier =
                Utils.ParseFloat(node, "rateOverDistanceMultiplier", mod.rateOverDistanceMultiplier);
        ...
        private void ParseExternalForces(ConfigNode node)
        {
            var mod = psystem.externalForces;
            mod.enabled = Utils.ParseBool(node, "enabled", true);
            mod.influenceFilter = ParseEnum(node, "influenceFilter", mod.influenceFilter);
            ...
            if (node.HasValue("forceFields"))
            {
                ...
                foreach (var ff in forceFields)
                {
                    ...
                    if (go != null) mod.AddInfluence(go.GetComponent<ParticleSystemForceField>());
                }
            }
        }

        private void ParseForceOverLifetime(ConfigNode node)
        {
            var mod = psystem.forceOverLifetime;

            mod.enabled = Utils.ParseBool(node, "enabled", true);
            mod.randomized = Utils.ParseBool(node, "randomized", mod.randomized);
            mod.space = ParseEnum(node, "space", mod.space);
        ...
        private void ParseInheritVelocity(ConfigNode node)
        {
        ...
        private void ParseLimitVelocityOverLifetime(ConfigNode node)
        {
            ...
            mod.multiplyDragByParticleSize =
                Utils.ParseBool(node, "multiplyDragByParticleSize", mod.multiplyDragByParticleSize);
            mod.multiplyDragByParticleVelocity =
                Utils.ParseBool(node, "multiplyDragByParticleVelocity", mod.multiplyDragByParticleVelocity);
            mod.space = ParseEnum(node, "space", mod.space);
        }

        private void ParseRotationBySpeed(ConfigNode node)
        {
        ...
        private void ParseRotationOverLifetime(ConfigNode node)
        {
        ...
        private void ParseShape(ConfigNode node)
        {
            ...
            mod.arcMode = ParseEnum(node, "arcMode", mod.arcMode);
            ...
            mod.meshShapeType = ParseEnum(node, "meshShapeType", mod.meshShapeType);
            mod.meshSpawnMode = ParseEnum(node, "meshSpawnMode", mod.meshSpawnMode);
            ...
            mod.radiusMode = ParseEnum(node, "radiusMode", mod.radiusMode);
            ...
            mod.shapeType = ParseEnum(node, "shapeType", mod.shapeType);
            ...
            mod.textureAlphaAffectsParticles =
                Utils.ParseBool(node, "textureAlphaAffectsParticles", mod.textureAlphaAffectsParticles);
            mod.textureBilinearFiltering = Utils.ParseBool(node, "textureBilinearFiltering", mod.textureBilinearFiltering);
            mod.textureClipChannel = ParseEnum(node, "textureClipChannel", mod.textureClipChannel);
            mod.textureClipThreshold = Utils.ParseFloat(node, "textureClipThreshold", mod.textureClipThreshold);
            mod.textureColorAffectsParticles =
                Utils.ParseBool(node, "textureColorAffectsParticles", mod.textureColorAffectsParticles);
        ...
        private void ParseSizeBySpeed(ConfigNode node)
        {
        ...
        private void ParseSizeOverLifetime(ConfigNode node)
        {
        ...
        private void ParseTrails(ConfigNode node)
        {
            ...
            mod.textureMode = ParseEnum(node, "textureMode", mod.textureMode);
        ...
        private void ParseTrigger(ConfigNode node)
        {
            var mod = psystem.trigger;

            mod.enabled = Utils.ParseBool(node, "enabled", true);
            mod.enter = ParseEnum(node, "enter", mod.enter);
            mod.exit = ParseEnum(node, "exit", mod.exit);
            mod.inside = ParseEnum(node, "inside", mod.inside);
            mod.outside = ParseEnum(node, "inside", mod.inside);
            mod.radiusScale = Utils.ParseFloat(node, "radiusScale", mod.radiusScale);
            if (node.HasValue("colliders"))
            {
                ...
                foreach (var c in colliders)
                {
                    ...
                    if (go != null)
                    {
                        ...
                        foreach (var col in cols) mod.SetCollider(index++, col);
                    }
                }
            }
        }

        private void ParseVelocityOverLifetime(ConfigNode node)
        {
        ...
        private Material ParseMaterial(ConfigNode node, string name, Material mat)
        {
            if (node.HasNode(name))
            {
                node = node.GetNode(name);
                Debug.Log($"[ParticleSystemCfg] ParseMaterial: {name}:{node}");
                var shaderName = node.GetValue("shader");
                Shader shader = null;
                if (shader == null && FindShader != null) shader = FindShader(shaderName);
        ...
        private void ParseRenderer(ConfigNode node)
        {
            psrenderer.alignment = ParseEnum(node, "alignment", psrenderer.alignment);
            ...
            psrenderer.maskInteraction = ParseEnum(node, "maskInteraction", psrenderer.maskInteraction);
            ...
            psrenderer.renderMode = ParseEnum(node, "renderMode", psrenderer.renderMode);
            ...
            psrenderer.sortMode = ParseEnum(node, "sortMode", psrenderer.sortMode);
            ...
            psrenderer.shadowCastingMode = ParseEnum(node, "shadowCastingMode", psrenderer.shadowCastingMode);
            psrenderer.shadowBias = Utils.ParseFloat(node, "shadowBias", psrenderer.shadowBias);
            ...
            if (node.HasValue("vertexStreams"))
            {
                ...
                foreach (var v in values) vertexStreams.Add(v.ToEnum(ParticleSystemVertexStream.Position));
        ...
        public void Load(ConfigNode node)
        {
            ...
            if (node.HasNode("main")) ParseMain(node.GetNode("main"));
            if (node.HasNode("collision")) ParseCollision(node.GetNode("collision"));
            if (node.HasNode("colorBySpeed")) ParseColorBySpeed(node.GetNode("colorBySpeed"));
            if (node.HasNode("colorOverLifetime")) ParseColorOverLifetime(node.GetNode("colorOverLifetime"));
            //if (node.HasNode ("customData")) {
            //	ParseCustomData (node.GetNode ("customData"));
            //}
            if (node.HasNode("emission")) ParseEmission(node.GetNode("emission"));
            if (node.HasNode("externalForces")) ParseExternalForces(node.GetNode("externalForces"));
            if (node.HasNode("forceOverLifetime")) ParseForceOverLifetime(node.GetNode("forceOverLifetime"));
            if (node.HasNode("inheritVelocity")) ParseInheritVelocity(node.GetNode("inheritVelocity"));
            ...
            //}
            if (node.HasNode("rotationBySpeed")) ParseRotationBySpeed(node.GetNode("rotationBySpeed"));
            if (node.HasNode("rotationOverLifetime")) ParseRotationOverLifetime(node.GetNode("rotationOverLifetime"));
            if (node.HasNode("shape")) ParseShape(node.GetNode("shape"));
            if (node.HasNode("sizeBySpeed")) ParseSizeBySpeed(node.GetNode("sizeBySpeed"));
            if (node.HasNode("sizeOverLifetime")) ParseSizeOverLifetime(node.GetNode("sizeOverLifetime"));
            ...
            //}
            if (node.HasNode("trails")) ParseTrails(node.GetNode("trails"));
            if (node.HasNode("trigger")) ParseTrigger(node.GetNode("trigger"));
            if (node.HasNode("velocityOverLifetime")) ParseVelocityOverLifetime(node.GetNode("velocityOverLifetime"));
            if (node.HasNode("renderer")) ParseRenderer(node.GetNode("renderer"));
        }
    }
}