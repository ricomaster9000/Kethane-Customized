...
public class MinMaxCurveCfg
{
    ...
    private AnimationCurve ParseCurve(ConfigNode node)
    {
        ...
        for (var i = 0; i < keys.Length; i++)
        {
            ...
            if (values.Length >= 4)
            {
                ...
                if (values.Length > 4) Debug.Log($"[MinMaxCurveCfg] key {i}: ignorning extra values: {values}");
            }
    ...
    public void Load(ConfigNode node)
    {
        ...
        else if (node.HasValue("constant"))
        {
            ...
            else if (values.Length >= 2)
            {
                ...
                if (values.Length > 2) Debug.Log($"[MinMaxCurveCfg] ignoring excess values: {constant}");
            }
            else
            {
        ...
        {
            Debug.Log(
                "[MinMaxCurveCfg] need one of constant (value), curve (node), or minCurve and maxCurve (node pair)");
        }
    }
}